  ...
  for (std::vector<std::string>::const_iterator iter = configs.begin();
    ...
    if (!customCommands.empty()) {
      ...
      for (std::vector<cmSourceFile const*>::iterator ccIter =
        ...
        customCommandTargetName << "-" << GeneratorTarget->GetName();
  ...
  std::string& command, const std::string& language)
{
  ...
  compileObjectVars.CMTargetName = this->GeneratorTarget->GetName().c_str();
      ...
      for (std::map<std::string, cmGlobalFastbuildGenerator::Detail::
        ...
        if (objectGroupLanguage == "RC") {
          ...
          const cmSourceFile* sourceFile =
            *(*command.sourceFiles.begin()).second.begin();
          const std::string& objectName =
            GeneratorTarget->GetObjectName(sourceFile);
          const std::string& sourceExtension =
            "." + sourceFile->GetExtension();
          std::string outputExtension = ".obj";
          if (objectName.find(sourceExtension) != std::string::npos) {
            outputExtension = objectName.substr(
              objectName.rfind(sourceExtension),
              objectName.size() - objectName.find(sourceExtension));
          }

          m_fileContext.WriteVariable(
            "CompilerOutputExtension",
            cmGlobalFastbuildGenerator::Quote(outputExtension));
        }

        std::map<std::string,
                 std::vector<const cmSourceFile*> >::const_iterator
          objectListIt;
        for (objectListIt = command.sourceFiles.begin();
          ...
          for (std::vector<const cmSourceFile*>::const_iterator sourceFileIt =
                 objectListIt->second.begin();
               sourceFileIt != objectListIt->second.end(); ++sourceFileIt) {
            const cmSourceFile* currentSourceFile = *sourceFileIt;
            if (!currentSourceFile->GetFullPath().empty()) {
              sourceFileVector.push_back(currentSourceFile->GetFullPath());
            }
          }
          m_fileContext.WriteArray(
            "CompilerInputFiles",
            cmGlobalFastbuildGenerator::Wrap(sourceFileVector, "'", "'"));
...
//        DetectLinkerLibPaths(linkPath, configName);
        ...
        if (GeneratorTarget->IsExecutableWithExports()) {
          ...
          if (defFile && defFile->DefFileGenerated) {
            linkFlags += defFileFlag + defFile->DefFile;
          }
        }
